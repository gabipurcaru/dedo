...
class Pad extends Component {
  constructor() {
    super();
    this.state = { editorHidden: true };
  }

  componentDidMount() {
    let editorHidden = true;
    if(typeof document !== 'undefined') {
      editorHidden = document.documentElement.clientWidth < FULL_SIZE;
    }
    this.setState({
      editorHidden,
    });
  ...
  _renderForm() {
    ...
    if(this.state.editorHidden) {
      ...
      toggleButton = (
        <Button
          className="fa fa-toggle-off Pad-toggle-editor"
          title="Show the editor"
          onClick={() => this.setState({ editorHidden: !this.state.editorHidden })}
          />
      );
    } else {
      className += " small";
  ...
  render() {
    ...
    let editor = (
      <Cell col={8}>
        {this._renderEditor()}
        <Button
          className="fa fa-toggle-on Pad-toggle-editor"
